...
async function miniAppInitHandler(request: Request, env: Env): Promise<Response> {
  ...
  // NOTE: group restriction removed per current requirements — this issues a
  // token for any valid Telegram user regardless of which chat (or no chat) the
  // Mini App was launched from.
...
router.post("/miniApp/init", miniAppInitHandler);
// Alias — some client setups call /init directly instead of /miniApp/init.
router.post("/init", miniAppInitHandler);
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    try {
      const response: Response = await router.fetch(request, env, ctx);
