...
import { HANDS_TO_WIN_MATCH, MAX_PLAYERS, RANKS, SUITS, TRICKS_PER_HAND, TRICKS_TO_WIN_HAND } from "./types";
...
export class HokmGameRoom {
  ...
  private async handleJoin(playerId: string, name: string, ws: WebSocket): Promise<void> {
    ...
    if (existingIdx === -1) {
      ...
      this.players.push({ id: playerId, name, seat: this.players.length });
    } else {
